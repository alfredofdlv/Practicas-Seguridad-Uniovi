...
def alta_usuario(nombre,contraseña):
    ...
    with open(r'D:\clase\Practicas-Seguridad-Uniovi\Criptografia\Autenticacion\usuarios.txt', mode='rb') as f_read:
        for linea in f_read:
            if nombre in linea.decode(): 
                print(f"El usuario {nombre} ya existe")
                return
    with open(r'D:\clase\Practicas-Seguridad-Uniovi\Criptografia\Autenticacion\usuarios.txt', mode='ab') as f_write:
