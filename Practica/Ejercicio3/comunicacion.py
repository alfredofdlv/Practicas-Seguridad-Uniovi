...
def empaquetar_con_firma(mensaje, priv_hex):
    ...
    msg_hash    = hashlib.sha256(mensaje)
    msg_hash_bytes= msg_hash.digest()
    priv_obj    = keys.PrivateKey(bytes.fromhex(priv_hex.replace('0x', '')))
    firma_bytes = priv_obj.sign_msg_hash(msg_hash_bytes).to_bytes()
    
    print('antes del digest',msg_hash)
    print('post-digest',msg_hash_bytes)
    print('de digest a texto',msg_hash.hexdigest())

    payload = {
        "mensaje": base64.b64encode(mensaje).decode(),
        "firma":   base64.b64encode(firma_bytes).decode(),
        "hash_msg": msg_hash.hexdigest()
    }
...
def desempaquetar_y_verificar(payload_bytes, pub_hex_emisor):
    ...
    hash_recuperado = payload["hash_msg"]
    ...
    firma_valida = pub_obj.verify_msg_hash(msg_hash, firma_obj)
    hash_valido = True if hash_recuperado== hashlib.sha256(mensaje).hexdigest() else False
    if firma_valida and hash_valido:
            es_valida=True
    else: 
        es_valida =False
        if not hash_valido:
            print('hash_recuperado',hash_recuperado)
            print("hash_original",hashlib.sha256(mensaje).hexdigest())
