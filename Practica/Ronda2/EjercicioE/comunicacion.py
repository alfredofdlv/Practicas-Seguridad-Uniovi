"""
Hibrido RSA + Fernet con firma interna y hash_msg ligado al token de sesion.
"""
...
def guardar_claves(priv, pub, nombre: str) -> None:
    ...
    with open(BASE_DIR / f"rsa_privada_{nombre}.pem", "wb") as f:
        f.write(
            priv.private_bytes(
    ...
    with open(BASE_DIR / f"rsa_publica_{nombre}.pem", "wb") as f:
        f.write(
            pub.public_bytes(
                serialization.Encoding.PEM,
...
def cargar_claves(nombre: str):
    ...
    with open(BASE_DIR / f"rsa_privada_{nombre}.pem", "rb") as f:
        priv = serialization.load_pem_private_key(f.read(), password=None)
    with open(BASE_DIR / f"rsa_publica_{nombre}.pem", "rb") as f:
